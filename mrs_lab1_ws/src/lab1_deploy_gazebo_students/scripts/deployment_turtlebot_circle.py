...
class Robot():
	def __init__(self, robot_id, num_robots, leader=False):
    	# Class attributes 
		self.robot_id = robot_id
		self.leader = leader
		self.num_robots = num_robots
		self.theta = np.linspace(0, 2*np.pi, self.num_robots, endpoint=False)[self.robot_id]
		self.t_local = 1/2.0
		self.x = 0.0
		self.y = 0.0
		self.neighbors = []
		self.available_neightbors = []
		self.inter_distance = 1
	...
	def read_parameters(self):
		try:
			...
			self.inter_distance = rospy.get_param("/inter_distance")
...
if __name__ == '__main__':
	...
	try:
		...
		my_naive_robot=Robot(robot_id, num_robots, leader)
		print('Finished!')
